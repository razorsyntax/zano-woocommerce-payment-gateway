 ...
 * Modified to work with zano-wallet-rpc wallet by razorsyntax
 */

defined('ABSPATH') || exit;

class Zano_Wallet_Rpc
{
    protected $url = null, $is_debug = false;
    protected $daemonUrl = null;
    ...
    public function __construct($pHost, $pPort, $daemonPort)
    {
    ...
    /**
     * Summary of request
     * @param mixed $pMethod string referring to the name of the method. eg. getaddress, getbalance
     * @param mixed $pParams
     * @return mixed
     */
    ...
    public function _runDaemon($method, $params = null)
    {
        $result = $this->requestDaeman($method, $params);
        return $result; //the result is returned as an array
    }

    /**
     * Summary of requestDaeman
     * @param mixed $pMethod string referring to the name of the method. eg. getaddress, getbalance
     * @param mixed $pParams
     * @return mixed
     */
    private function requestDaeman($pMethod, $pParams)
    {
        static $requestId = 0;
        ...
        // Response (method invocation)
        $responseMessage = $this->getDaemonResponse($request, $pMethod);
    ...
    protected function &getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
            if (is_admin())
                echo '[ERROR] Failed to connect to zano-wallet-rpc at ' . $this->host . ' port ' . $this->port . '</br>';
        }
    ...
    protected function &getDaemonResponse(&$pRequest, $apiMethod)
    {
        // do the actual connection
        $ch = curl_init();
        if (!$ch) {
        ...
        if ($apiMethod == 'getheight') {
            curl_setopt($ch, CURLOPT_URL, $this->daemonUrl . '/getheight');
        } else {
            curl_setopt($ch, CURLOPT_URL, $this->daemonUrl);
        }
        ...
        if (0 < curl_errno($ch)) {
            if (is_admin())
                echo '[ERROR] Failed to connect to zano-wallet-rpc at ' . $this->host . ' port ' . $this->daemonPort . '</br>';
        }
        // close the connection
        curl_close($ch);
        return $response;
    ...
    /*
     * The following functions can all be called to interact with the Zano RPC wallet
    ...
    public function split_integrated_address($integrated_address)
    {
        if (!isset($integrated_address)) {
            if (is_admin())
                echo "Error: Integrated_Address must not be null";
        } else {
    ...
    public function make_uri($address, $amount, $recipient_name = null, $description = null)
    {
        ...
        $new_amount = $amount * ZANO_GATEWAY_ATOMIC_UNITS_POW;
