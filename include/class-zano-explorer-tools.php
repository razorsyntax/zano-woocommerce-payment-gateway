...
class Zano_Explorer_Tools
{
    private $url;
    public function __construct($testnet = false)
    {
        $this->url = $testnet ? ZANO_GATEWAY_TESTNET_EXPLORER_URL : ZANO_GATEWAY_MAINNET_EXPLORER_URL;
    ...
    {
        $data = $this->call_api('/api/get_info');
        if($data['status'] == 'OK')
